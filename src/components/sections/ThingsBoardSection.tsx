...
import { mockBots, mockThingsBoardUsers, mockDeviceTelemetry } from '../../data/mockData';
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="bg-white/60 dark:bg-zinc-900/40 backdrop-blur-md border-[0.5px] border-zinc-200/50 dark:border-zinc-800/40 rounded-xl shadow-sm p-5">
            <p className="text-sm text-zinc-500 dark:text-zinc-400">{s.label}</p>
            <p className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100 mt-1">{s.value}</p>
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-2">{s.sub}</p>
          </div>
        ))}
      </div>
      ...
      <Card title="Bot Legend">
        <div className="flex flex-wrap gap-4">
          {mockBots.map((bot) => (
            <div key={bot.botId} className={`flex items-center gap-2 text-sm ${bot.status === 'coming-soon' ? 'opacity-50 italic' : ''}`}>
              <span className={`w-2.5 h-2.5 rounded-full ${bot.colorDot}`} />
              <span className="text-zinc-700 dark:text-zinc-300">{bot.botName}</span>
              <span className="text-zinc-400 dark:text-zinc-500 text-xs">{bot.botId}</span>
            </div>
          ))}
        </div>
      </Card>
            ...
            {users.map((u) => {
              ...
              return (
                <tr
                  ...
                  className={`cursor-pointer border-b border-zinc-50 dark:border-zinc-800/50 last:border-b-0 transition-colors ${isSelected ? 'bg-indigo-50/60 dark:bg-indigo-500/10' : 'hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40'}`}
                >
                  ...
                  <td className="py-3 pr-4">
                    {bot ? (
                      <span className="inline-flex items-center gap-1.5 text-xs text-zinc-600 dark:text-zinc-300">
                        <span className={`w-2 h-2 rounded-full ${bot.colorDot}`} /> {bot.botName}
                      </span>
                    ) : (
        ...
        <Card title={`${selectedUser.name} — Assigned Bot & Devices`}>
          {selectedBot ? (
            <p className="text-sm text-zinc-600 dark:text-zinc-300 mb-4">
              Assigned to <span className="font-medium text-indigo-600 dark:text-indigo-400">{selectedBot.botName}</span> ({selectedBot.botId})
            </p>
          ) : (
      ...
      {draft && (
        ...
        >
          <div
            className="w-full max-w-lg bg-white dark:bg-zinc-900 border-[0.5px] border-zinc-200/60 dark:border-zinc-800/60 rounded-xl shadow-xl p-6 max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5">User name</label>
                <input
                  type="text"
                  value={draft.name}
                  onChange={(e) => setDraft({ ...draft, name: e.target.value })}
                  placeholder="e.g. Alice Chen"
                  className="w-full px-3 py-2 rounded-lg text-sm bg-white dark:bg-zinc-800 border-[0.5px] border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5">Assign bot</label>
                <select
                  value={draft.botId ?? ''}
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5">Assign devices</label>
                  ...
                  {knownDevices.map((id) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-md text-xs font-medium border-[0.5px] transition-colors ${
                          checked
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-lg text-sm bg-white dark:bg-zinc-800 border-[0.5px] border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
                  />
