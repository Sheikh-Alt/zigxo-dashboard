import { Building2, Users, Activity, Zap, AlertTriangle, Info, AlertCircle, ArrowRight } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import type { OverviewKPI, AlertItem, RecentTenantRow, RecentUserRow } from '../../types';

const kpiIconMap: Record<string, LucideIcon> = { Building2, Users, Activity, Zap };
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {kpis.map((kpi) => {
          ...
          return (
            <div
              key={kpi.id}
              className="bg-white/60 dark:bg-zinc-900/40 backdrop-blur-md border-[0.5px] border-zinc-200/50 dark:border-zinc-800/40 rounded-xl shadow-sm p-5 flex flex-col gap-3"
            >
              ...
              <p className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">{kpi.value}</p>
              <p className={`text-xs font-medium ${deltaColor}`}>{kpi.delta}</p>
            </div>
          );
        })}
      </div>
