...
export default function TopicsSection() {
  ...
  useEffect(() => {
    if (selectedId == null) { setUploadedFiles([]); return; }
    setLoadingFiles(true);
    fetch(`${API}/${selectedId}/files`)
      .then(r => r.json())
      .then((files: Array<{ id: number; original_name: string; size_bytes: number }>) => {
        setUploadedFiles(files.map(f => ({ id: f.id, name: f.original_name, size: formatSize(f.size_bytes) })));
      })
      .catch(e => console.error('Failed to load files', e))
      .finally(() => setLoadingFiles(false));
  }, [selectedId]);

  // ── Load agent config into local state when agent changes ──────────────────

  useEffect(() => {
    const agent = agents.find(a => a.id === selectedId);
    if (!agent) return;
    setSystemPrompt(agent.system_prompt || '');
    setSystemPromptEnabled(!!agent.system_prompt);
    const starters = agent.conversation_starters || [];
    setStarters(starters);
    setConversationStartersEnabled(starters.length > 0);
    setPendingFiles([]);
  }, [selectedId, agents]);
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    setIsDragging(false);
    const next: PendingFile[] = Array.from(e.dataTransfer.files).map(f => ({
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const next: PendingFile[] = Array.from(e.target.files).map(f => ({
      id: `f-${Date.now()}-${Math.random()}`,
      name: f.name,
      size: formatSize(f.size),
      file: f,
  ...
  // ── Upload pending files to the DB ─────────────────────────────────────────
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (const pf of pendingFiles) {
        ...
        const res = await fetch(`${API}/${selectedId}/files`, { method: 'POST', body: form });
        if (res.ok) {
          const f = await res.json();
          uploaded.push({ id: f.id, name: f.original_name, size: formatSize(f.size_bytes) });
        }
      }
  ...
  const handleRemoveUploadedFile = async (fileId: number) => {
    ...
    await fetch(`${API}/${selectedId}/files/${fileId}`, { method: 'DELETE' });
    setUploadedFiles(prev => prev.filter(f => f.id !== fileId));
  };

  // ── URL / text (local only for now) ───────────────────────────────────────
    ...
    try {
      const res = await fetch(`${API}/${selectedId}`, {
        ...
        body: JSON.stringify({
          ...
          conversation_starters: conversationStartersEnabled ? starters : [],
        }),
      });
      if (res.ok) {
        const updated = await res.json();
        setAgents(prev => prev.map(a => a.id === selectedId ? { ...a, ...updated } : a));
        setSaved(true);
        setTimeout(() => setSaved(false), 2000);
      }
    } catch (e) {
      console.error('Save instructions failed', e);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleReset = () => {
    ...
    setConversationStartersEnabled(false);
    setStarters([]);
    setStarterInput('');
  };
  ...
  const handleAddAgent = async () => {
    ...
    try {
      const res = await fetch(API, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newAgentName.trim(), description: newAgentDesc.trim() }),
      });
  ...
  const saveEditAgent = async () => {
    ...
    setSavingEdit(true);
    try {
      const res = await fetch(`${API}/${editingAgent.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: editAgentName.trim() || editingAgent.name, description: editAgentDesc.trim() }),
      });
      const updated = await res.json();
      setAgents(prev => prev.map(a => a.id === editingAgent.id ? { ...a, ...updated } : a));
      setEditingAgent(null);
    } catch (e) {
      console.error('Edit agent failed', e);
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      await fetch(`${API}/${deletingAgent.id}`, { method: 'DELETE' });
      const remaining = agents.filter(a => a.id !== deletingAgent.id);
      setAgents(remaining);
      if (selectedId === deletingAgent.id) setSelectedId(remaining[0]?.id ?? null);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { setEditingAgent(null); setDeletingAgent(null); }
    };
  ...
  return (
    <div className="space-y-4 relative">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {loadingAgents ? (
          <div className="col-span-full text-xs text-zinc-400 py-4 text-center">Loading agents…</div>
        ) : (
          <>
            {agents.map(agent => {
              ...
              return (
                <div key={agent.id} className="relative group/card">
                  ...
                  <div className="absolute bottom-2 right-2 flex gap-0.5 opacity-0 group-hover/card:opacity-100 pointer-events-none group-hover/card:pointer-events-auto transition-opacity duration-150 z-10">
                    <button onClick={() => openEditAgent(agent)} title="Edit agent" className="p-1 rounded-md text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors">
                      <IconPencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setDeletingAgent(agent)} title="Delete agent" className="p-1 rounded-md text-zinc-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors">
            ...
            {!showAddAgent ? (
              ...
              <div className="p-4 rounded-xl border border-zinc-200/60 dark:border-zinc-700/50 bg-white/60 dark:bg-zinc-900/40 space-y-2.5">
                <input
                  ...
                  className="w-full text-sm border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
                <textarea
                  value={newAgentDesc}
                  ...
                  rows={2}
                  className="w-full text-sm border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                />
                <div className="flex gap-2">
                  <button onClick={handleAddAgent} disabled={addingAgent || !newAgentName.trim()} className="text-xs font-semibold text-white bg-indigo-600 px-3 py-1.5 rounded-md hover:bg-indigo-700 disabled:opacity-50 transition-colors">
                    {addingAgent ? 'Creating…' : 'Create'}
                  </button>
                  <button onClick={() => { setShowAddAgent(false); setNewAgentName(''); setNewAgentDesc(''); }} className="text-xs font-semibold text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors">Cancel</button>
                </div>
              </div>
            )}
          </>
      ...
      {selectedAgent && (
        <div className="bg-white/60 dark:bg-zinc-900/40 backdrop-blur-md border-[0.5px] border-zinc-200/50 dark:border-zinc-800/40 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800/70">
            ...
            <button className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-xs font-semibold hover:bg-zinc-700 dark:hover:bg-white transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-zinc-100 dark:divide-zinc-800/70">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Files stored in database — used to train the AI</p>
              </div>
              ...
              <div className="flex gap-0.5 p-0.5 bg-zinc-100 dark:bg-zinc-800/80 rounded-lg w-fit border border-zinc-200/60 dark:border-zinc-700/50">
                {(['files', 'url', 'text'] as const).map(tab => (
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-xs font-semibold transition-all duration-150 ${dataTab === tab ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200'}`}
                  >
                    {tab === 'files' && <><IconFile className="w-3.5 h-3.5" /> Files</>}
                    {tab === 'url'   && <><IconLink className="w-3.5 h-3.5" /> URL</>}
                    {tab === 'text'  && <><IconText className="w-3.5 h-3.5" /> Text</>}
                  </button>
                ))}
              </div>
              ...
              {dataTab === 'files' && (
                <>
                        ...
                        {pendingFiles.map(f => (
                          <div key={f.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-amber-50/60 dark:bg-amber-500/10 border border-amber-200/60 dark:border-amber-700/40 group">
                            ...
                            <div className="flex items-center gap-2 ml-2 shrink-0">
                              ...
                              <button onClick={() => setPendingFiles(p => p.filter(x => x.id !== f.id))} className="opacity-0 group-hover:opacity-100 text-zinc-400 hover:text-rose-500 transition-all"><IconX className="w-3.5 h-3.5" /></button>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div>
                    <p className="text-[11px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mb-2">
                      {loadingFiles ? 'Loading…' : uploadedFiles.length > 0 ? `Stored in DB (${uploadedFiles.length})` : 'Stored in DB'}
                    </p>
                    {uploadedFiles.length > 0 ? (
                      <div className="space-y-1.5">
                        {uploadedFiles.map(f => (
                          <div key={f.id} className="flex items-center justify-between py-2 px-3 rounded-lg border border-zinc-200/60 dark:border-zinc-700/50 group hover:bg-zinc-50/60 dark:hover:bg-zinc-800/40 transition-colors">
                            ...
                            <div className="flex items-center gap-2 ml-2 shrink-0">
                              <span className="text-[11px] text-zinc-400">{f.size}</span>
                              <button onClick={() => handleRemoveUploadedFile(f.id)} className="opacity-0 group-hover:opacity-100 text-zinc-400 hover:text-rose-500 transition-all" title="Remove"><IconX className="w-3.5 h-3.5" /></button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ) : !loadingFiles && (
                      <p className="text-xs text-zinc-400 dark:text-zinc-500 italic py-2">No files stored yet</p>
                    )}
                  </div>

                  <div className="flex items-center gap-2 pt-1 border-t border-zinc-100 dark:border-zinc-800/60">
                    <button onClick={() => setPendingFiles([])} disabled={pendingFiles.length === 0} className="px-4 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Clear all</button>
                    <button onClick={handleUpload} disabled={pendingFiles.length === 0 || uploading} className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                      {uploading ? 'Uploading…' : <> Upload <IconArrow className="w-3.5 h-3.5" /> </>}
                    </button>
                  </div>
                </>
              )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-sm border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                    <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-1.5">Content will be fetched.</p>
                  </div>
                  <div className="flex items-center gap-2 pt-1 border-t border-zinc-100 dark:border-zinc-800/60">
                    <button onClick={() => setUrlInput('')} disabled={!urlInput} className="px-4 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Clear</button>
                    <button onClick={handleFetchUrl} disabled={!urlInput.trim()} className={`inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed ${urlFetched ? 'bg-emerald-600 text-white' : 'bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-white'}`}>
              ...
              {dataTab === 'text' && (
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea rows={8} value={textContent} onChange={e => setTextContent(e.target.value)} placeholder="Paste any text content here…" className="w-full text-sm border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600" />
                    <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-1.5">Plain text, markdown, or structured content.</p>
                  </div>
                  <div className="flex items-center gap-2 pt-1 border-t border-zinc-100 dark:border-zinc-800/60">
                    <button onClick={() => setTextContent('')} disabled={!textContent} className="px-4 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Clear all</button>
                    <button onClick={handleSaveText} disabled={!textContent.trim()} className={`inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed ${textSaved ? 'bg-emerald-600 text-white' : 'bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-white'}`}>
            ...
            <div className="p-6 flex flex-col gap-4">
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Define how this agent thinks and responds — saved to database</p>
              </div>

              <div className="flex gap-0.5 p-0.5 bg-zinc-100 dark:bg-zinc-800/80 rounded-lg w-fit border border-zinc-200/60 dark:border-zinc-700/50">
                {(['prompt', 'references'] as const).map(tab => (
                  <button key={tab} onClick={() => setInstructionTab(tab)} className={`px-3.5 py-1.5 rounded-md text-xs font-semibold transition-all ${instructionTab === tab ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200'}`}>
              ...
              {instructionTab === 'prompt' && (
                <div className="flex flex-col gap-4 flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="e.g. What does temp_in mean?"
                      ...
                      />
                      <button onClick={handleAddStarter} disabled={!conversationStartersEnabled || !starterInput.trim()} className="w-9 h-9 flex items-center justify-center rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-300 dark:hover:border-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                        <IconPlus className="w-4 h-4" />
                      </button>
                    </div>
                    {starters.length > 0 && (
                      <div className="space-y-1.5">
                        {starters.map((s, i) => (
                          <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-indigo-50/60 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 group">
                            ...
                            <button onClick={() => setStarters(p => p.filter((_, j) => j !== i))} className="opacity-0 group-hover:opacity-100 ml-2 shrink-0 text-indigo-400 hover:text-rose-500 transition-all"><IconX className="w-3.5 h-3.5" /></button>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="flex items-center justify-end gap-2 mt-auto pt-3 border-t border-zinc-100 dark:border-zinc-800/70">
                    <button onClick={handleReset} className="px-4 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">Reset</button>
                    <button onClick={handleSaveInstructions} disabled={saving} className={`inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg transition-all ${saved ? 'bg-emerald-600 text-white' : 'bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50'}`}>
                      {saving ? 'Saving…' : saved ? 'Saved ✓' : <> Save instructions <IconArrow className="w-3.5 h-3.5" /> </>}
                    </button>
                  </div>
                </div>
              )}
      ...
      {!loadingAgents && agents.length === 0 && (
        <div className="text-center py-16 text-zinc-400 dark:text-zinc-500">
          <AgentIcon className="w-10 h-10 mx-auto mb-3 opacity-40" />
          <p className="text-sm font-medium">No agents yet</p>
          <p className="text-xs mt-1">Click &ldquo;Add New Agent&rdquo; to get started</p>
        </div>
      )}
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-zinc-950 rounded-2xl shadow-2xl border border-zinc-200/60 dark:border-zinc-800/60 p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={editAgentName} onChange={e => setEditAgentName(e.target.value)} onKeyDown={e => e.key === 'Enter' && saveEditAgent()} autoFocus className="w-full px-3 py-2 text-sm rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-700/50 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/40" />
              </div>
              <div>
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5">Description</label>
                <textarea rows={2} value={editAgentDesc} onChange={e => setEditAgentDesc(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-700/50 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 resize-none" />
              </div>
            </div>
            <div className="flex justify-end gap-2.5 mt-5">
              ...
              <button onClick={saveEditAgent} disabled={savingEdit} className="px-4 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors shadow-sm disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-zinc-950 rounded-2xl shadow-2xl border border-zinc-200/60 dark:border-zinc-800/60 p-6">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed mb-6">
              Are you sure you want to delete <span className="font-semibold text-zinc-700 dark:text-zinc-300">{deletingAgent.name}</span>? All uploaded files will be permanently removed.
            </p>
            <div className="flex justify-end gap-2.5">
              ...
              <button onClick={confirmDeleteAgent} disabled={confirmingDelete} className="px-4 py-2 text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 rounded-lg transition-colors shadow-sm disabled:opacity-50">
